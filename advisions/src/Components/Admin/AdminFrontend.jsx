  ...
  const columns = [
    { field: '_id', headerName: 'ID', width: 70 },
    {
      field: 'pic', headerName: 'Background Images', width: 130, renderCell: ({ row }) => (
        row.pic ? <a href={`/assets/img/${row.pic}`} target='_blank' rel="noreferrer"><img src={`/assets/img/${row.pic}`} style={{ height: "100px", width: "100px" }} alt="" /></a> : null
      )
    },
    { field: 'des', headerName: 'Description', width: 130 },
    {
      field: "edit",
      headerName: "Edit",
      sortable: false,
      renderCell: ({ row }) =>
        <Button onClick={() => {
          navigate("/admin-update-frontend/" + row._id)
        }}>
    ...
    },
    {
      ...
      <Button onClick={() => {
        if (window.confirm("Are Your Sure You Want to Delete that Item :")) {
          dispatch(deleteFrontend({ _id: row._id }))
        }
      }}>
  ...
  useEffect(() => {
    const getAPIData = () => {
      ...
      if (allfrontends.length)
        setFrontend(allfrontends);
    };

    getAPIData();
  }, [dispatch, allfrontends.length, allfrontends]);
  return (
    <>
      {/* <!-- Hero Start --> */}
